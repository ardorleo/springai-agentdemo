 ...
 * 驱动 UI 的 ↻ 提示行（spec §3.2「L1 的 UI 可见性」）；L1 桥侧 maxAttempts 恒传 7（总尝试数）。无绑定为 no-op（传 null）。
 *
 * <p><b>attempt 口径（勿混，spec 第 8 轮 #8）</b>：即将进行的<b>第几次尝试</b>，取值 2..7——
 * 与 UI 文案 {@code (2/7·传输)} 的尝试序号一致；与 {@link RetryPolicy#backoffMsAfter} 的
 * 1 基「第 n 次尝试<b>失败后</b>」序号差 1。
 *
 * <p><b>时序纪律（R6-m3）</b>：经 {@code backoffRetry(...)} 的退避前回调触发——它在退避
...
public interface RetryReporter {
     ...
     * 一次 L1 重试已排定。
     *
     * @param attempt   即将进行的第几次尝试（2..7；首重试 = 2）
