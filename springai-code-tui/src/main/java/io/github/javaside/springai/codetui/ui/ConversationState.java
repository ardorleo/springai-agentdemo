...
public final class ConversationState implements AgentListener, UiChangeSource {
     ...
     * 渲染线程每次重绘都要对整段残行做预览+语法高亮+折行——O(百万) 的字符串操作会把渲染线程拖死，
    ...
    /**
     * 同上，但本次最多取 {@code maxLines} 个完整逻辑行，其余<b>留在缓冲区</b>等下一批。
     *
     * <p><b>为什么必须能限量</b>：模型一次吐出一个几千行的代码块（或工具结果回显）时，无参版本会把
     * 全部完整行一次交给渲染线程 println——那是<b>每批 pty 写入限速唯一漏掉的一条路径</b>，偏偏
     * 「窗口正在输出」走的就是它。一批灌几千行 = 几百 KB 突发：渲染线程被占住（按键排队，用户感知
     * 「打字卡死」），终端也被推进它自己的崩溃区。
     *
