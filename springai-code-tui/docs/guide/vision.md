# 视觉输入（图片真正进模型）

> 本页内容对应 README 的「视觉输入」章节。


支持视觉的模型能**真正看见**图片：你自己在输入框里贴的图，以及工具产生的图（`Read` 一张 png、MCP 截图工具的返回），都会作为图片交给模型，而不只是一行文件路径。

### 自己贴图

**输入框里直接写图片路径就行**，不需要 `@` 之类的标记：

```
> 这个报错界面是怎么回事 docs/bug.png
  ⏎ 已附带 1 张图片（bug.png）  · Ctrl+X 取消
```

也可以**把文件从访达/桌面拖进终端**——终端不传文件、只把路径当粘贴插到光标处，所以拖拽走的是同一条路，没有额外机制。

- **拖拽带空格的文件名也认**：macOS 中文截图的默认文件名就带空格。反斜杠转义（`~/Desktop/截屏\ 2026-08-02.png`，**macOS 实测就是这个形态**）、单引号包裹、双引号包裹三种都吃，`~` 会展开，中文名照常。多文件拖拽插进来的多个路径也会各自识别。
- **最多 3 张**——就是下方硬上限里那个「每请求用户贴图 ≤3 张」，不另设一个。超出的不附，附件行会写明「另有 N 张超出上限未附」，不会静默丢掉让你以为都附上了。
- **判定看魔数不看扩展名**：一个叫 `a.png` 的文本文件**不会**被当图片附上。反之路径不独立成词（`see docs/bug.pngx`）、文件不存在、读不了，都当普通文本处理，不报错也不提示。

#### 会误附，所以给了 `Ctrl+X`

裸路径自动识别**不可能只在你想附图时命中**。`把 docs/bug.png 复制到 tmp/` 这种句子里，路径独立成词、文件存在、魔数是图片——三条判据全中，**必然被误认为附件**。

这个代价是设计时明确接受的：**由「附件行当场可见 + `Ctrl+X` 一键撤销」承担，不靠规则去消灭它**。加语法标记能消灭误附，但也就同时消灭了「拖进来即可」。

所以附件行是**边打边刷新**的，回车之前你就看得见附了几张；不想附就按 `Ctrl+X`，那一行变成 `⏎ 已取消附件 · 路径仅作普通文本发送`，本次提交不再重新识别（否则再敲一个字符它就回来了）。清空输入框或提交后取消状态复位。

> **取消附件不会删掉你输入框里的那段路径**——最典型的误附场景（`把 docs/bug.png 复制到 tmp/`）里，你**就是要**跟模型说这个路径，删掉等于毁掉你的话。所以取消后路径仍在，只是不再当图片发；那一行会明说这一点，免得你以为没生效。若确实想连路径一起去掉，自己删就是了。

> 用 `Ctrl+X` 而非 `Ctrl+D`：本项目实现了 readline 键位，`Ctrl+D` 在 readline 里是「删除光标处字符」/ 空行时 EOF，绑成取消附件会跟肌肉记忆打架；而 `Ctrl+X` 在 readline 里是**前缀键**（`Ctrl+X Ctrl+E` 之类），单按通常无动作，不撞肌肉记忆，且本项目未占用（已用 `Ctrl+A/E/W/U/K`、`Alt+B/F`）。macOS 上「剪切」是 `Cmd+X` 不是 `Ctrl+X`，被系统抢的概率也低。
>
> ⚠️ **某些全局热键会抢占按键**，撞上时症状是「按了没反应」或**弹出别的窗口**——不是本功能坏了。已知一例：Chrome 的 Gemini 扩展把 `Ctrl+G` 注册成 OS 级全局热键，键在终端应用看到它之前就被拿走（取消键原本就是 `Ctrl+G`，正是因此换掉的）。真撞上时先关掉抢键的那个程序确认一下。

#### 模型不支持视觉时会拦住不发

当前模型不在下方名单里而你附了图，这条消息**不会发出去**，状态栏提示换模型，且**输入框内容原地保留**——`/model` 切完直接回车重发即可，不用把那段话连同路径重贴一遍。

#### 项目内的图指原文件，项目外的图复制一份

- **项目内**的图（`docs/design.png`）→ 引用块里就是原路径，**不复制**。你回头更新了这个文件，模型再看到的就是**新版**；模型 `Read` 它的行为也与自己 `Read` 一张图完全一致。
- **项目外**的图（拖进来的桌面截图）→ **复制一份进 `.codetui/artifacts/`**，引用块指向副本。这不是偏好：引用块解析器会拒掉一切指向项目外的路径（防的是外部内容注入 `path: ../../../etc/id_rsa`），不复制的话那张图会**无声地永远兑现不了**。

`-c` 恢复会话时，存储里那段引用块会渲成单行 `📎 bug.png (1440×900)`，不会把机器格式甩你一屏。

### 哪些模型能看见

按模型 id **前缀**匹配，命中即支持：

| provider | 前缀 |
|---|---|
| OpenAI | `gpt-5.`、`gpt-4o`、`o4-`、`gpt-6-astra` |
| Anthropic | `claude-` |
| 通义千问 | `qwen-vl`、`qwen2-vl`、`qwen2.5-vl`、`qwen3-vl` |
| 智谱 | `glm-4v`、`glm-4.1v`、`glm-4.5v`、`glm-4.6v`、`glm-5.3-flash`、`glm-5v` |
| DeepSeek | `deepseek-v4-flash-vision` |

**不在名单里的一律判「不支持」**——误判「不支持」只是拦住你、提示可见可改；误判「支持」是把图真发出去吃一个看不懂的 400。

> **内置清单里能直接用上视觉的：OpenAI、Anthropic、DeepSeek 与智谱四家**（`gpt-6-astra`（2026-09-03
> 发布，1.05M 上下文，前缀只精确到 astra——gpt-6 家族其他成员未经核实不进名单）、`gpt-5.6-*` / `gpt-5.5` /
> `gpt-5.4`、`claude-*`、`deepseek-v4-flash-vision-exp`、`glm-5.3-flash`——最后这个是 2026-08-26 上线的
> GLM-5 系首个原生多模态模型，1M 上下文，价格为 glm-5.3 的 1/10）。
> 千问的内置清单（`qwen3.7-max`/`qwen3.7-plus`/`qwen3.6-flash`/`qwen3-coder-next`）仍无视觉模型——
> 要在千问用视觉，得自己用 `DASHSCOPE_MODELS` 配一个 `-vl` 系 id；智谱的 `glm-4.6v`（2025-12 上线，
> 首个原生 Function Call 的视觉线，Flash 档免费）同理走 `ZHIPU_MODELS`。

### 图片从不进会话记忆

落盘的永远是一段文本引用块（`id` / `mime_type` / `dimensions` / `name` / `path` / `delivery`），图片字节只在**出站请求组装的最后一刻**被挂上，且只挂**当轮**的。

因此**聊多久都不会累积上下文**：聊 100 轮、看过 50 张图，请求里也只有当轮那几张。想让模型重看历史图片，让它 `Read` 引用里的 `path` 即可——那条路本来就通（Read 一张图 → 工具结果出现引用 → 落在当轮 → 被兑现），不需要额外命令。

### 硬上限（这是设计的核心）

```
每请求   用户贴图 ≤3 张 + 工具产图 ≤1 张（最新那张） + 16k 视觉 token
每回合   累计兑现 ≤12 张·次（工具图）/ 36 张·次（用户图）
```

用户图**先**过预算且保底不淘汰，工具图只在彼此之间竞争取最新一张：反过来会让「照这张稿子改」的稿子被随后 `Read` 的图挤掉。

> **token 按 provider 各自口径估算**（见 `ImageProfile`）。早期对所有家套用 `宽 × 高 / 750`，对 DeepSeek 实测高估 3.6 倍（同一张 2442×1146 截图：旧公式 3731、真机约 1000），把每请求上限从「6 张」压成「3 张」——「额度感觉太少」即源于此。
> 新口径下同一张图 DeepSeek 约 556、Anthropic 约 1512、OpenAI 约 1190。

被上限挡下的图不会静默消失，引用块的 `delivery` 行会写明结局：`delivered` / `not_in_view`（Read 一次就能看）/ `budget_exceeded`（当轮图太多，单独 Read 它）/ `turn_budget_exhausted`（本回合额度用尽，下个回合再说）/ `reference_only`（当前模型看不见，别白 Read）。

> **图片只在「首次出现」的那一轮自动发送一次**，此后模型要重看需自己 `Read`。
> 早先每轮重传，实测第 1 轮的截图陪跑 11 轮把额度吃光，导致模型真正需要看图时 `Read` 也拿不到——推送饿死了拉取。
> 代价：模型需要用图时多一次 `Read` 往返。

`/context` 面板把视觉占用**单列**，不混进文本估算：`ContextStats` 估的是会话存储里的文本，而图片从不进存储，两个数字合并只会让「为什么请求比面板大」变得无法解释。

### 图片处理

| 格式 | 处理 |
|---|---|
| PNG / JPEG / GIF | 长边超出该 provider 上限时等比缩后再发（**磁盘原件不动**；DeepSeek 1300 / Anthropic 1568 / OpenAI·通义 2048）；缩后仍超 4MB 转 JPEG 再压一次 |
| BMP / TIFF | 解码后转码成 PNG |
| **WebP** | **原样发、不缩**（JDK 解不了，但各家 API 收）；>4MB 则不兑现 |
| **HEIC / AVIF** | **发不出去**，只留引用（HEIC 是 iPhone 照片默认格式，会真遇到） |

- 超过 **5000 万像素**的图直接不兑现——判定只读文件头拿尺寸，**绝不先解码再判断**（200MB 的 PNG 解成 `BufferedImage` 是 GB 级，那一下就把进程带走了）。
- 引用块里的 `sha` 是**原图**的，而发出去的是缩后 / 转码后的字节，**两者不同**。sha 用于寻址与去重，**不用于校验发出内容**。

### 逃生口

```bash
export CODETUI_VISION=off     # 全局关闭整条视觉链路（引用照常，零行为变化）
```

DeepSeek 专属传输开关（默认内联；`files` 走 Files API 的 `file_id` 引用，单图上限 64 MiB，
上传失败自动降级内联）：

```bash
export DEEPSEEK_VISION_TRANSPORT=files
```

DeepSeek 每张图按服务端自动缩放后计费，**单张最多 384 token**（远低于本工具按
`宽×高/750` 的估算，预算只会更宽松）。

#### 为什么默认内联（以及它的已知局限）

**计费与传输方式无关**——无论 base64 内联还是 Files API，DeepSeek 都按「每图最多 384 token」
计费，files 并不省钱。选择内联默认，是因为本项目图片已由 `ImagePreparer` 压到 ≤4 MiB / 长边
1568（远低于 32 MiB 内联上限），内联形态零额外往返、无文件生命周期、失败面最小（字节随请求
走，没有「上传成功但引用 400」的归因难题）。

**已知局限（当前未修，按需再议）**：同一回合内每次工具迭代都会重发请求，内联意味着**同一张图
的字节被反复送**（无状态请求的固有代价，见上文「同一回合内每次工具迭代都会重传当轮的图」）。
`DEEPSEEK_VISION_TRANSPORT=files` 可以在这一层省下传输量——上传一次拿 `file_id`，后续迭代只
引用 id，请求体从 ~MiB 级降到几十字节（省的是带宽/延迟，**不是钱**）。当前保持内联默认是权衡
后的决定；若图片体积或网络往返成为实际痛点，可把默认翻转或让 `file_id` 持久化复用。

### ⚠️ 验证范围（别当成「五家都能用」）

「工具结果 → 合成一条 user 消息（带图）」这个消息序列，只在**一个本地兼容中转网关 + `gpt-5.6-sol`** 上做过真机验证：发纯红 / 纯绿 / 纯蓝三张图，模型三次全答对；**对照组**（同样序列但不挂图）明确回答「只看到图片文件引用，无法看到实际图像内容」。

**`api.openai.com` 原生端点，以及 Anthropic / 千问两家，完全没有验证过**（本机没有对应 API key）。它们第一次真用时**仍可能返回 400**。请不要把这一节读成「已支持五家」。

**智谱**：`glm-5.3-flash` 的内联 base64 通道、流式工具调用、图+工具复合请求均已真机验证
（`ZhipuVisionSmokeTest`，纯红/纯蓝图 → 颜色答对、工具触发）——spring-ai-openai 通路对智谱 v4
无需任何 HTTP 改写（与 DeepSeek 的区别：DeepSeek 的 SDK 丢 Media，必须走改写层）。
上述验证在智谱 Coding Plan 端点（`ZHIPU_BASE_URL=https://open.bigmodel.cn/api/coding/paas/v4`）取得——
key 的资源包绑在该端点，普通端点 `https://open.bigmodel.cn/api/paas/v4` 对这把 key 是 429 余额不足。
普通计费端点未实际调用成功；它与 Coding Plan 端点共用 v4 兼容协议，因此上述协议层结论预计可迁移，但仍需独立真机确认。

**DeepSeek**：`deepseek-v4-flash-vision-exp` 的<b>内联 base64 通道</b>已真机验证（纯红图 →
模型答对颜色）。Files API 通道（`DEEPSEEK_VISION_TRANSPORT=files`）已做单测覆盖，未真机验证。

### 不支持的场景

- **终端里显示不了图片**：模型截的图、生成的图，你在 TUI 里只会看到一行文件路径，要看得自己去开文件。系统提示里已明确要求模型**别写 `![](路径)` 这类 markdown 图片语法**（终端里只会显示成一串原始文本），改为直接给路径 + 用文字讲清画面上有什么。最近一张图另有一条稳定软链 `.codetui/artifacts/latest.<ext>`，`open .codetui/artifacts/latest.png` 即可，不用去复制那串 64 位 sha（只对图片建、建不出来就没有，不影响图本身落盘）。
- **`Bash` 生成图片文件不会自动产生引用**：`python plot.py` 写出 `chart.png`，工具结果是纯文本，不会凭空多出一个引用块。这是**有意为之**——输出里出现一个路径不代表模型想看它；模型想看就 `Read` 它，那条路是通的。
- **`.codetui/artifacts/` 超过 500MB 时按最旧优先删除**，不做引用扫描——误删的后果只是模型 `Read` 时拿到「文件不存在」，可恢复。清理**只在启动时扫一次**：单次长会话里产出再多也要等下次启动才回收。
- **不能 `Ctrl+V` 直接粘剪贴板里的图**：贴图只认路径（手写或拖文件）。读系统剪贴板要各平台一套外部命令，且无头环境里测不了，故不做。截图先存成文件再拖进来。
- **视频不投递**：`supportsVideoInput` 是保留字段，恒为 false。

