...
这让历史图片的自动视觉成本为零。模型需要重看历史图时调用 `Read`，新工具结果位于当前锚点之后，因此重新进入可兑现范围。

### 7.1.1 高水位线：图片只推一次

当轮范围决定“哪些图**有资格**被兑现”，但仅此还不够：图片字节不进会话历史，所以一条引用只要还在当轮范围内，本来会**每轮请求都被重新兑现**，直到把回合额度吃光。

实测的后果：第 1 轮工具产出的一张截图，在此后 11 轮（模型早已转向别的工具调用）仍被重发，把回合额度耗尽，导致模型**真正需要看图时 `Read` 也拿不到**——推送把拉取饿死了。

因此再叠一条**消息下标高水位线**：

```text
只有 [上一轮处理到的下标, 当前消息数) 这段新增消息里的引用才推
```

- 用户图 → 回合首轮推一次；
- 工具图 → 产出它的那一轮推一次；
- 模型后来 `Read` 老图 → 那个工具结果是**新的**（下标 ≥ 高水位线），照样能拿到图 —— 否则“按需”就成了“永远拿不到”。

换回合时高水位线复位为 0。`高水位线` 的必要性在于：引用文本本身不携带“是否推过”的信息，而会话存储里那份必须是干净的（永远不该出现 `delivered`），所以这个状态只能由兑现器自己持有。

不推的那些引用保持原文本里的 `not_in_view`——这对本轮是**准确**的（本轮确实没发，模型可 `Read` 取回），写成 `delivered` 反而是假话。

改动的实测效果（同一场景）：自动重发消耗从 12 张·次降到 1 张·次，模型随后 `Read` 一张真正需要的图可以拿到。

代价：模型需要用图时会多一次 `Read` 往返；首次推送后若那一轮没看到（如响应被截断），不会自动补发，需自己 `Read`。
...
| 实际格式 | 处理方式 | 原因 |
| --- | --- | --- |
| PNG / JPEG | 小图原样；长边超过该 provider 的上限时等比缩放 | Provider 支持，缩放可降低字节和视觉 token |
...
### 8.2 最大长边按 provider 取值

大截图通常远高于模型理解所需分辨率，缩到各家自己的处理上限即可保留界面文字和结构，同时减少上传字节与视觉 token。

各家能接受的分辨率差别很大，统一取一个值会两头受损——对处理尺寸小的 provider 白缩一次，对大的则净丢细节（截图小字变糊）：

| Provider | 长边上限 | 依据 |
| --- | --- | --- |
| DeepSeek | 1300 | 官方缩放目标；发更多像素不带来额外信息 |
| Anthropic | 1568 | 标准档上限（高分辨率档 2576 未接入） |
| OpenAI | 2048 | `detail:high` 容许范围 |
| 通义千问 | 2048 | 受其默认 `max_pixels` 约束，更大无意义 |
| 未知（智谱/网关） | 1568 | 无官方口径，取保守值 |
...
每请求：
  用户图片最多 3 张
  工具图片最多 1 张
  视觉估算最多 16000 token（按 provider 各自口径估算）
...
每请求 token 上限并不能阻止一次工具循环调用模型很多次。单回合额度限制的是累计重传成本。

预算按当前回合 key 分桶，避免共享装饰器时不同回合互相消耗额度。计数表有界，只保留有限数量的回合 key。

**用户图片与工具图片各有独立额度**，不共享同一个计数器。共享时工具循环会挤掉用户图的额度：实测 2 张用户图在第 7 轮就被掐掉，而一个回合可能跑几十轮工具调用。用户贴的 1–3 张图是他这一轮的全部意图，中途静默消失会让“照这张图改”在回合后半段直接失效。

| 来源 | 每回合上限 | 依据 |
| --- | --- | --- |
| 工具图 | 12 张·次 | 工具图每请求只发最新一张，故上限即轮数；约 21.6k token |
| 用户图 | 36 张·次 | 用户图最多 3 张同时在场，每轮消耗是工具图的 1–3 倍；约 65k token |

用户图额度取 36（= 3 张 × 12 轮）是为了与工具图的口径对齐。

注意：图片改为「首次推一次」（见 7.1.1）之后，用户图不再被自动重发消耗——用户图额度实际只在**模型反复 `Read`** 时才会触及，属兜底而非主路径。

两者都<b>必须有界</b>：图片不进会话历史、每轮请求都要重发，所以成本随轮数线性上涨——实测用户图不设限时，2 张图跑 30 轮就是 60 张·次 ≈ 112.8k token。

可用 `CODETUI_VISION_TURN_BUDGET` 覆盖工具图额度，用 `CODETUI_VISION_USER_TURN_BUDGET` 覆盖用户图额度（非法值安全回落默认）。

**额度耗尽后 `Read` 也拿不回图片**，因为它按回合分桶、`Read` 产生的引用仍落在同一桶里。因此工具执行期需要知道额度状态，并在耗尽时把引用写成 `turn_budget_exhausted`，而不是继续写“`Read` 一次就能看”——后者会让模型反复 `Read` 却始终看不到，最后只能回答“我看不见”。这条判断要求工具侧与出站侧共用同一个预算实例（装配见 `AgentTools`）。

### 9.4 视觉 token 按 provider 各自口径估算

早期实现用 Anthropic 的 `宽 × 高 / 750` 估算**所有** provider。对 DeepSeek 实测高估 3.6 倍（同一张 2442×1146 截图：旧公式 3731，真机约 1000），于是 6000 的每请求上限实际只放得下 3 张图而本可放 6 张。**高估看似安全，实际是白白限制功能**——表现为用户觉得「额度太少」，而这与真实成本无关。

因此改为按 provider 估算（见 `ImageProfile`）：

| Provider | 口径 | 可信度 |
| --- | --- | --- |
| DeepSeek | 极小图取 226 下限；`226 + 像素/2400`；封顶 1024 | **真机实测拟合** |
| Anthropic | `⌈w/28⌉ × ⌈h/28⌉`，封顶 1568 | 官方文档公式 |
| OpenAI 通路 | `⌈宽/32⌉ × ⌈高/32⌉ × 1.2`（按原始像素） | **真机实测**（聚合网关） |
| 通义千问 | `w̄×h̄ / 1024 + 2`（含 `max_pixels` 缩放） | 官方文档 |
| 智谱 GLM | `min(6086, max(18, ⌈w/28⌉ × ⌈h/28⌉))` | **真机实测** |
| 未知（网关混杂） | 沿用 `宽 × 高 / 750` | 无官方依据，取保守 |

**OpenAI 档用的是实测口径而非官方文档**：本项目 `OPENAI_BASE_URL` 常指向第三方聚合网关，其计费实现与 OpenAI 官方 `base + 512px分块×tile` 不同——官方口径在本通路会低估（2048×961 官方 1190、实测 2381）。换官方端点需重新校准。端到端对照：2442×1146 截图 → 我们发出 2048×961、估算 2381、真机实测 2381，零误差。

**智谱档为实测拟合**（官方未公布公式）：28×28 分块，地板 18、封顶 6086。端到端对照：2442×1146 截图 → 发出 1568×736、估算 1512、真机实测 1458（差 3.7%）。Anthropic / OpenAI / 通义只有文档口径，**没有真机实测**。

改后的实测效果（同一张 2442×1146 截图）：DeepSeek 从「估算 1538」降到「556」，每请求可容纳的图从 3 张升到 25 张量级。
